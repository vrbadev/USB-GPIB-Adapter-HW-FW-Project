...
_USB_STRING_(wsVendor, L"STMicroelectronics")
_USB_STRING_(wsProd, L"ST USB-GPIB")
_USB_STRING_(wsSN, L"V1.0")
...
int usb_printf_(const char* format, ...)
{
	va_list va;
  va_start(va, format);
	char buffer[128];
  ...
  va_end(va);
	USBLIB_SendData(2, (uint16_t*) buffer, ret);
  return ret;
}
